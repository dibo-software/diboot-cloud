...
public class UploadFileServiceImpl extends BaseServiceImpl<UploadFileMapper, UploadFile> implements UploadFileService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void bindRelObjId(UploadFileBindRefDTO uploadFileBindRefDTO) throws Exception {
        //如果不存在需要绑定的么清除所有当前关联的所有文件
        if (V.isEmpty(uploadFileBindRefDTO.getFileUuidList())) {
            this.update(
                    Wrappers.<UploadFile>lambdaUpdate()
                            .set(true, UploadFile::isDeleted, true)
                            .eq(UploadFile::getRelObjId, uploadFileBindRefDTO.getRelObjId())
                            .eq(UploadFile::getRelObjType, uploadFileBindRefDTO.getRelObjType())
            );
            return;
        }
        // 删除 relObjId + relObjType下的 并且不包含fileIdList的file。
        this.update(
                Wrappers.<UploadFile>lambdaUpdate()
                        ...
                        .eq(UploadFile::getRelObjId, uploadFileBindRefDTO.getRelObjId())
                        .eq(UploadFile::getRelObjType, uploadFileBindRefDTO.getRelObjType())
                        .notIn(UploadFile::getUuid, uploadFileBindRefDTO.getFileUuidList())
        );
        // 绑定绑定数据
        this.update(
                Wrappers.<UploadFile>lambdaUpdate()
                        .set(true, UploadFile::getRelObjId, uploadFileBindRefDTO.getRelObjId())
                        .in(UploadFile::getUuid, uploadFileBindRefDTO.getFileUuidList())
        );
    }
