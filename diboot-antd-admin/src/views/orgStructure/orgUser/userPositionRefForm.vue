...
export default {
  ...
  methods: {
    ...
    loadPositionKvList () {
      dibootApi.get('/auth-server/iam/position/kvList').then(res => {
        if (res.code === 0) {
    ...
    loadOrgList () {
      dibootApi.get('/auth-server/iam/org/tree').then(res => {
        if (res.code === 0) {
    ...
    async onSubmit () {
      try {
        ...
        const res = await dibootApi.post('/auth-server/iam/position/batchUpdateUserPositionRelations', values)
