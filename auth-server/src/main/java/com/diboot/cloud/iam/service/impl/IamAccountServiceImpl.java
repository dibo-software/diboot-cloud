...
import com.diboot.cloud.iam.mapper.IamAccountMapper;
...
public class IamAccountServiceImpl extends BaseIamServiceImpl<IamAccountMapper, IamAccount> implements IamAccountService {
    ...
    public boolean createEntities(List<IamAccount> accountList){
        if(V.notEmpty(accountList)){
            accountList.stream().forEach(account->{
                // 加密
                if(Cons.DICTCODE_AUTH_TYPE.PWD.name().equals(account.getAuthType())){
                    String encryptedPwd = passwordEncoder.encode(account.getAuthSecret());
                    account.setAuthSecret(encryptedPwd);
                }
            });
        }
        // 保存
    ...
    public boolean changePwd(ChangePwdDTO changePwdDTO, IamAccount iamAccount) throws Exception {
        ...
        // 验证旧密码是否正确
        String oldAuthSecret = passwordEncoder.encode(changePwdDTO.getOldPassword());
